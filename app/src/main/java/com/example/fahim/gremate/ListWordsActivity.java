...
public class ListWordsActivity extends NavDrawerActivity {
    ...
    private ArrayList<WordWithId> wordwIDs;
...
//            finish();
//        }
//        wsId = extras.getString("wordset_key");
...
//                final ArrayList<ListWithId> tempList = getOtherLists(false);
//                if(tempList.size() == 0){
...
//                            DB.newWord(word, currentListId, currentListName, wsId, allListId);
//                        }
//                    }
//                });
...
//                                    DB.deleteList(currentListId, wsId, true);
//                                    Intent intent = new Intent(ListWordsActivity.this, WordSetActivity.class);
//                                    intent.addFlags(Intent.FLAG_ACTIVITY_CLEAR_TOP | Intent.FLAG_ACTIVITY_NEW_TASK);
//                                    startActivity(intent);
//                                    ListWordsActivity.this.finish();
//                                }
//                                else{
//                                    DB.deleteList(currentListId, wsId, false);
...
//        q = ref.orderByChild("wordSet").equalTo(wsId);
...
//            @Override
//            public void onDataChange(DataSnapshot dataSnapshot) {
//                wordwIDs = new ArrayList<WordWithId>();
//                for(DataSnapshot ds: dataSnapshot.getChildren()){
...
//                        int j = 0;
//                        for(int i=0; i<4; i++){
//                            String listKey = DB.newList("List "+(i+1), wsId);
//                            int lim = (wordwIDs.size() * (i+1)) / 4 ;
//                            for( ; j<lim; j++) {
//                                Word nword = wordwIDs.get(j).toWord();
//                                nword.setCloneOf(wordwIDs.get(j).getId());
...
//    }
//
//    private ArrayList<ListWithId> getOtherLists(boolean removeListAll){
//        ArrayList<ListWithId> tempList = new ArrayList<>();
...
//        if (wordwIDs == null) return;
//        getListSortOrder();
//        if(sortOrder == 11){Collections.sort(wordwIDs, WordWithId.recAdded_Asc);}
//        else if (sortOrder == 12) {Collections.sort(wordwIDs, WordWithId.recAdded_Dsc);}
//        else if (sortOrder == 21) {Collections.sort(wordwIDs, WordWithId.recViewed_Asc);}
//        else if (sortOrder == 22) {Collections.sort(wordwIDs, WordWithId.recViewed_Dsc);}
//        else if (sortOrder == 31) {Collections.sort(wordwIDs, WordWithId.alphabetical_Asc);}
//        else if (sortOrder == 32) {Collections.sort(wordwIDs, WordWithId.alphabetical_Dsc);}
//        else if (sortOrder == 41) {Collections.sort(wordwIDs, WordWithId.difficulty_Asc);}
//        else if (sortOrder == 42) {Collections.sort(wordwIDs, WordWithId.difficulty_Dsc);}
//
//        if(resetList) resetListState();
//        else if(rvAdapter != null)setListState();
//        rvAdapter = new WordAdapter(wordwIDs, friends, ListWordsActivity.this, wsId, allListId, currentListId, userName);
...
//        SharedPreferences preferences = PreferenceManager.getDefaultSharedPreferences(this);
//        preferences.edit().putString(wsId, currentListId).apply();
//    }
//
//    private void restoreList(){
//        final SharedPreferences preferences = PreferenceManager.getDefaultSharedPreferences(this);
//        lastSavedList = preferences.getString(wsId, null);
//    }
//
...
//                        intent = new Intent(ListWordsActivity.this, FriendActivity.class);
//                        intent.addFlags(Intent.FLAG_ACTIVITY_CLEAR_TOP | Intent.FLAG_ACTIVITY_NEW_TASK);
//                        startActivity(intent);
//                        break;
//                }
